...
TEST VERSION:
This code isolates subject 0018's data into a separate "0018_temp" directory
instead of the normal "0018" directory. This was used for:

1. Testing the extraction pipeline on subject 0018 without overwriting production data
2. Keeping test outputs separate from real extracted data
3. Easy cleanup - you can delete the entire "0018_temp" folder after testing
"""
...
class StreamingExtractor:
    ...
    def extract_performance(self, smc_file, subject_id, performance):
        ...
        # Create output directory - use 0018_temp for testing
        if subject_id == "0018":
            output_dir = Path(self.config['storage']['output_dir']) / "0018_temp" / performance
        else:
    ...
    def process_subject(self, subject_id):
        ...
        if subject_id == "0018":
            subject_dir = Path(self.config['storage']['output_dir']) / "0018_temp"
        else:
